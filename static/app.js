...
const shorts={federal:'RF',fgts:'FG',trabalhista:'JT',falencia:'TJ',estadual_mg:'MG',estadual_sp:'SP',municipal:'SR',municipal_congonhal:'CG'};
...
const congonhalFields={nome_usuario:'#congonhal-nome-usuario',cpf_usuario:'#congonhal-cpf-usuario'};
...
function render(){
  ...
  $('#results').innerHTML='<p class="status-summary">'+esc(summary)+'</p>'+results.map(r=>{
    ...
    const preview=r.status==='encontrada'&&r.document_url?'<section class="pdf-preview" aria-label="Prévia da certidão em PDF"><div class="pdf-preview-bar"><span>Prévia da certidão</span><a href="'+esc(r.document_url)+'" target="_blank" rel="noopener noreferrer">Abrir PDF</a></div><iframe title="Prévia em PDF - '+esc(service.label)+'" src="'+esc(r.document_url)+'#toolbar=0&navpanes=0"></iframe></section>':'';
    return '<article class="result-card"><div class="result-top"><div class="result-identity"><span class="result-icon">'+shorts[r.service]+'</span><div><h3>'+esc(service.label)+'</h3><small>'+esc(service.issuer)+'</small></div></div><span class="badge '+tone(r.status)+(r.status==='consultando'?' spinning':'')+'">'+esc(label)+'</span></div><p>'+esc(r.message)+'</p>'+(r.evidence?'<div class="evidence-label">RETORNO DO ÓRGÃO</div><blockquote class="evidence">'+esc(r.evidence)+'</blockquote>':'')+(r.diagnostic?'<p class="result-diagnostic">'+esc(r.diagnostic)+'</p>':'')+preview+download+(!pending(r.status)?'<div class="result-footer"><span>'+esc(stage+' · '+at)+'</span><a class="button secondary" href="'+esc(r.url)+'" target="_blank" rel="noopener noreferrer">Abrir portal ↗</a></div>':'')+'</article>';
  }).join('');
...
function toggleExtraFields(){const services=selectedServices(),falencia=$('#falencia-fields'),congonhal=$('#congonhal-fields');if(falencia)falencia.hidden=!services.includes('falencia');if(congonhal)congonhal.hidden=!services.includes('municipal_congonhal');}
...
function congonhalPayload(){const data={};for(const [key,selector] of Object.entries(congonhalFields)){const value=$(selector)?.value.trim();if(value)data[key]=value;}return data;}
$('#query-form').addEventListener('submit',async event=>{
  ...
  if(services.includes('municipal_congonhal'))body.congonhal=congonhalPayload();
...
async function init(){
  try{
    ...
    $('#services').addEventListener('change',toggleExtraFields);toggleExtraFields();
    $('#query-fields').disabled=false;
    const previous=sessionStorage.getItem('certifica-consulta');
    if(previous){try{run=await fetchJson('/api/consultations/'+previous);$('#cnpj').value=formatCnpj(run.cnpj);document.querySelectorAll('input[name=service]').forEach(input=>input.checked=input.value in run.results);toggleExtraFields();render();if(run.running)pollTimer=setTimeout(poll,1000);}catch{sessionStorage.removeItem('certifica-consulta');}}
