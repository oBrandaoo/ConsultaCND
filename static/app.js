...
function render(){
  ...
  $('#results').innerHTML='<p class="status-summary">'+summary+'</p>'+results.map(r=>{
    ...
    return '<article class="result-card"><div class="result-top"><div class="result-identity"><span class="result-icon">'+shorts[r.service]+'</span><div><h3>'+esc(service.label)+'</h3><small>'+esc(r.service==='municipal'?run.city:service.issuer)+'</small></div></div><span class="badge '+tone(r.status)+(r.status==='consultando'?' spinning':'')+'">'+esc(label)+'</span></div><p>'+esc(r.message)+'</p>'+(r.evidence?'<div class="evidence-label">RETORNO DO ÓRGÃO</div><blockquote class="evidence">'+esc(r.evidence)+'</blockquote>':'')+download+assist+(!pending(r.status)?'<div class="result-footer"><span>'+esc(stage+' · '+at)+'</span><a class="button secondary" href="'+esc(r.url)+'" target="_blank" rel="noopener noreferrer">Abrir portal ↗</a></div>':'')+'</article>';
  }).join('');
